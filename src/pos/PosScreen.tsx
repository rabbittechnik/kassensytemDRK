...
/** Synthetischer Kategorie-Tab für Pfand-Auszahlung (keine echte categoryId in der DB). */
const PFAND_TAB_ID = '__pfand_tab__'
...
export function PosScreen({
  ...
  const isPfandTab = activeCat === PFAND_TAB_ID
  const categoryForProducts = isPfandTab ? null : (activeCat ?? firstCatId)

  const dexProducts = useLiveQuery(async () => {
    if (!categoryForProducts) return []
    const rows = await db.products.where('categoryId').equals(categoryForProducts).toArray()
    return rows
      .filter((p) => p.active)
  ...
  const openDepositRedeemModal = useCallback(
    async (preset: DepositOption | null = null) => {
      if (!demoMode) {
      ...
      setDepositQty(1)
      setDepositNote('')
      const pick = preset ?? depositOptions[0]
      if (pick) {
        setDepositName(pick.name)
        setDepositAmountCents(pick.amountCents)
        setDepositType(pick.type)
      }
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (checkoutChoiceOpen && e.key === 'Escape') {
        e.preventDefault()
        setCheckoutChoiceOpen(false)
        return
      }
      if (checkoutChoiceOpen && !e.repeat) {
    ...
    <div className="pos-screen-root flex h-full min-h-0 min-w-0 flex-col bg-black font-bold text-white">
          ...
          <nav className="flex flex-shrink-0 flex-wrap gap-2 md:gap-3" aria-label="Kategorien">
            {categories.map((c) => (
              <button
                key={String(c.id)}
                type="button"
                className={tabCls(Boolean(categoryForProducts === String(c.id)))}
                onClick={() => setActiveCat(String(c.id))}
              >
                {c.name}
              </button>
            ))}
            <button type="button" className={tabCls(isPfandTab)} onClick={() => setActiveCat(PFAND_TAB_ID)}>
              Pfand
            </button>
          </nav>
                ...
                <>
                  {depositOptions.length === 0 ?
                    <div className="col-span-full flex min-h-[12rem] flex-col items-center justify-center gap-4 px-2 text-center">
                      ...
                      <button
                        ...
                        className="rounded-xl border-2 border-sky-500/60 bg-sky-950/30 px-6 py-3 text-sm font-black uppercase tracking-wide text-sky-100 shadow-[0_0_22px_rgba(14,165,233,.22)] hover:bg-sky-950/45"
                      >
                        Pfand auszahlen
                      </button>
                    </div>
                  : depositOptions.map((opt) => (
                    <button
                      ...
                      type="button"
                      onClick={() => void openDepositRedeemModal(opt)}
                      className="flex min-h-[5.85rem] w-full min-w-0 shrink-0 flex-col items-center justify-center gap-1 rounded-xl border-2 border-sky-500/70 bg-sky-950/20 px-3 py-3 text-center shadow-[0_0_22px_rgba(14,165,233,.22)] transition-transform active:scale-[0.98] hover:bg-sky-950/35 sm:min-h-[6.35rem]"
                    >
                      <span className="text-lg" aria-hidden>
                        ♻
                      </span>
                      <span className="text-sm font-black uppercase leading-snug text-white sm:text-[0.9375rem]">
                        {opt.name}
                      </span>
                      <span className="text-base font-black tabular-nums text-[#FFD700] sm:text-lg">
                        {formatMoney(opt.amountCents)} / Stück
                      </span>
                    </button>
                  ))
                  }
        ...
        >
          ...
          <div className="space-y-1 border-t border-[#ff003c]/35 bg-black/40 px-4 py-4">
            ...
            <button
              ...
              onClick={() => setCheckoutChoiceOpen(true)}
              className="mt-3 flex min-h-[52px] w-full flex-col items-center justify-center gap-0.5 rounded-xl border-2 border-[#FFD700]/80 bg-gradient-to-br from-[#ff003c]/90 to-red-950/95 px-4 py-3 font-black uppercase tracking-wide text-white shadow-[0_0_28px_rgba(255,215,0,0.35)] transition enabled:active:scale-[0.98] disabled:opacity-35"
            >
      ...
      {checkoutChoiceOpen && (
        <div
          ...
          onClick={() => setCheckoutChoiceOpen(false)}
          role="presentation"
        >
          <div
          ...
          >
            ...
            <div className="mt-4 flex flex-col gap-2">
              <button
                ...
                className="checkout-button flex min-h-[52px] flex-col items-center justify-center gap-0.5 rounded-xl border-2 border-emerald-400/70 bg-emerald-900/35 font-black uppercase text-emerald-50 transition enabled:active:scale-[0.99] disabled:opacity-35"
              >
                <span className="checkout-button-title text-sm">Barzahlung</span>
                <span className="checkout-button-subtitle text-[11px] font-bold text-emerald-200">
                  F12 / Enter
                </span>
              </button>
              <button
                ...
                className="checkout-button flex min-h-[52px] flex-col items-center justify-center gap-0.5 rounded-xl border-2 border-cyan-400/70 bg-cyan-950/30 font-black uppercase text-cyan-100 transition enabled:active:scale-[0.99] disabled:opacity-35"
              >
                <span className="checkout-button-title text-sm">Kartenzahlung</span>
                <span className="checkout-button-subtitle text-[11px] font-bold text-cyan-200">F11</span>
              </button>
              <button
                type="button"
                disabled={
              ...
              >
                <span className="checkout-button-title text-sm">Auf Rechnung</span>
              </button>
              <button
                type="button"
                disabled={!saleAvailability.canSell || cart.length === 0}
                ...
                className="checkout-button flex min-h-[52px] flex-col items-center justify-center gap-0.5 rounded-xl border-2 border-orange-400/70 bg-orange-900/30 font-black uppercase text-orange-100 transition enabled:active:scale-[0.99] disabled:opacity-35"
              >
                <span className="checkout-button-title text-sm leading-tight">Helferverpflegung</span>
                <span className="checkout-button-subtitle text-[11px] font-bold text-orange-200">
