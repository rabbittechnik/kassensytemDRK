...
export function CashTenderModal(props: {
  ...
  const { totalCents, subtitleHint, onCancel, onConfirm } = props
  ...
  function submit() {
    ...
    if (given === null || given < totalCents) {
      setErr('Betrag zu niedrig oder ungültig.')
      return
    }

    void onConfirm(given)
  }
  ...
  const changePreview =
    (() => {
      ...
      return given !== null && given >= totalCents ? given - totalCents : null
    })()
  ...
  return (
    ...
    >
      <div className="panel-dlrg max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-[#ff003c]/50 p-6 shadow-[0_0_60px_rgba(255,0,60,0.35)]">
        ...
        <p id="cash-hint" className="mt-2 text-sm font-semibold text-neutral-400">
          Gegeben-Betrag (EUR) eingeben. Rückgeld wird angezeigt. Esc = zurück ohne
          Verbuchen.
        </p>

        {subtitleHint ? (
          <p className="mt-2 rounded-lg border border-cyan-500/30 bg-cyan-950/20 px-3 py-2 text-xs font-semibold text-cyan-200/90">
            {subtitleHint}
          </p>
        ) : null}
        ...
        <div className="mt-5 rounded-2xl border-2 border-[#FFD700]/40 bg-black/60 px-4 py-4 text-center">
          ...
          <div className="mt-1 text-3xl font-black text-[#FFD700]">{formatMoney(totalCents)}</div>
        </div>
